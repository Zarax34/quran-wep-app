...
The configuration file is located at `mobile_app/codemagic.yaml`.

**Important:**
Since this project folder was generated by an AI agent, it **does not contain** the `android/` and `ios/` platform folders by default (as they require the `flutter` CLI tool).

**Before pushing to Codemagic, you MUST run:**
```bash
cd mobile_app
flutter create . --platforms android,ios
```
*This command will generate the necessary Android and iOS build files without overwriting your `lib/` code.*
